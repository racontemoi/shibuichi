...
{
	public function __construct($controller, sfDate $start_date)
	{
	 	Requirements::customScript("
			var controller_url_segment = '" . $controller->URLSegment . "'; 
			var current_url_segment = '" . Controller::curr()->URLSegment . "';
...
class LiveCalendarWidget extends ViewableData
{
  ...
  private $anchor_start;
  private $anchor_end;
  private $date_counter;
  private $calendarHeader;
  private $startDayOfMonth;
  ...
  private function getQuickMonthLink()
  {
    $d = new sfDate();
    return $this->calendar->AbsoluteLink().$d->firstDayOfMonth()->format('Ym');
  }
  
  private function getQuickWeekLink()
  {
  	$d = new sfDate();
    return $this->calendar->AbsoluteLink().$d->firstDayOfWeek()->format('Ymd')."/".$d->finalDayOfWeek()->format('Ymd');

  }
  
  private function getQuickWeekendLink()
  {
  	$d = new sfDate();
  	...
  	// Before Friday? Advance. Otherwise, it's Friday, so leave it alone.
 		...
 		return $this->calendar->AbsoluteLink().$d->format('Ymd')."/".$d->addDay(2)->format('Ymd');
  }
  
  
  private function getEventsFor($start, $end)
  {
